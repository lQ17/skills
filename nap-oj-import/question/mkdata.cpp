...
void mk_in() {
    // 创建testcases目录
    if (system("rm -rf testcases/*.in testcases/*.out testcases/*.zip") != 0) {
        cerr << "清理旧数据失败" << endl;
    }
    
    if (system("mkdir -p testcases") != 0) {
        cerr << "创建testcases目录失败" << endl;
        return;
    }

    cout << "开始生成输入数据..." << endl;
    for (int i = 1; i <= TEST_CASES; ++i) {
        string in_name = "testcases/" + to_string(i) + ".in";
...
void mk_out() {
    ...
    for (int i = 1; i <= TEST_CASES; ++i) {
        string in_name = "testcases/" + to_string(i) + ".in";
        string out_name = "testcases/" + to_string(i) + ".out";
